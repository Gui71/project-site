...
DATA_FINAL = fuso_horario.localize(datetime(2025, 4, 17, 17, 59)) # Ajuste considerando ano, mes, dia, hora, min
...
def truncar_texto(pdf, texto, largura_maxima):
    # Armazena o texto original para verificar se foi truncado
...
# Função para corrigir caracteres incompatíveis
...
def exportar_pdf_individual(id):
    ...
    dados_inscrito = [
        ("RG:", fix_encoding(inscrito[2]), "CPF:", fix_encoding(inscrito[3])),
        ("Celular:", fix_encoding(inscrito[4]), "Email:", fix_encoding(inscrito[5])),
        ("Empresa:", fix_encoding(inscrito[6]), "Bairro:", fix_encoding(inscrito[7])),
        ("Autorização de uso de dados:", "Sim" if inscrito[8] == "on" else "Não", "Autorização de uso da Imagem:", "Sim" if inscrito[9] == "on" else "Não"),
    ]

    pdf.cell(190, 10, f"Nome: {fix_encoding(inscrito[1])}", 1, align='C')
    pdf.ln()
...
def exportar_pdf(pagina):
    ...
    for inscrito in inscritos_pagina:
        dados_inscrito = [
            ("RG:", fix_encoding(inscrito[2]), "CPF:", fix_encoding(inscrito[3])), # Correção de caracteres incompatíveis
            ("Celular:", fix_encoding(inscrito[4]), "Email:", fix_encoding(inscrito[5])), # Correção de caracteres incompatíveis
            ("Empresa:", fix_encoding(inscrito[6]), "Bairro:", fix_encoding(inscrito[7])), # Correção de caracteres incompatíveis
    ...
    for nome, dados_inscrito in zip([fix_encoding(inscrito[1]) for inscrito in inscritos_pagina], dados_pdf): # Correção de caracteres incompatíveis
